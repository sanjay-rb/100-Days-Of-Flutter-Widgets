...
class _Day7FadeTransitionState extends State<Day7FadeTransition>
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () {
        _controller.forward(); // ? start the animation.... [opacity = 0.0(begin) to 1.0(end)]
      },
      onDoubleTap: () {
        _controller.reverse(); // ? reverse the animation.... [opacity = 1.0(end) to 0.0(begin)]
      },
      onLongPress: () {
        _controller.repeat(); // ? repeat the animation.... loop[opacity = 0.0(begin) to 1.0(end)]
      },
      child: Center(
        child: Stack(
          alignment: AlignmentDirectional.center,
          children: <Widget>[
            FadeTransition( // ? what is FadeAnimation....
              opacity: _animation,
